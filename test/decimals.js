...
describe("Option: decimals", () => {
	...
	it("Should round integer part if decimals is zero", () => {
		ziffer.format(1234.56, { decimals: 0 }).should.equal("1235");
	});

	it("Should round decimal part if decimals is above zero", () => {
		ziffer.format(0.345, { decimals: 0 }).should.equal("0");
		ziffer.format(0.345, { decimals: 2 }).should.equal("0,35");
		ziffer.format(1234.56, { decimals: 1 }).should.equal("1234,6");
		ziffer.format(1234.567, { decimals: 2 }).should.equal("1234,57");
		ziffer.format(1234.564, { decimals: 2 }).should.equal("1234,56");
	});
	...
	it("Should add zeros if decimals is above zero", () => {
		...
		ziffer.format(123.45, { decimals: 0 }).should.equal("123");
